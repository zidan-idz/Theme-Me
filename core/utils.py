...
def check_termux():
    if "PREFIX" not in os.environ and os.name != 'nt':
         print_warning("It seems you are not running in Termux.")
         print_warning("Some features might not work or could be dangerous.")
...
def update_tool():
    ...
    if os.path.isdir(".git"):
        try:
            print("")
            os.system("git pull")
            # print_success('Proses selesai tanpa masalah')
            print_success("Pembaruan diperiksa/diterapkan.")
        except Exception as e:
            # print_error('Proses selesai dengan masalah')
